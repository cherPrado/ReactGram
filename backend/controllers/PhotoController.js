...
const deletePhoto = async (req, res) => {
    ...
    try {
        ...
        if (!photo.userId.equals(reqUser._id)) {
            return res.status(422).json({ errors: ["Você não tem permissão para excluir esta foto."] });
        }
        ...
        return res.status(500).json({ error: "Ocorreu um erro, por favor tente novamente mais tarde." });
    }
};
...
const getPhotoById = async (req, res) => {
     ...
     if(!photo){
        res.status(404).json({errors: ["Foo não encontrada."]})
     }
